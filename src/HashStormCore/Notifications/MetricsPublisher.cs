...
namespace HashStormCore.Notifications;
...
public class MetricsPublisher : BackgroundService
{
    ...
    {
        poolConnectionsGauge = Metrics.CreateGauge("HashStormCore_pool_connections", "Number of connections per pool", new GaugeConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        poolHashrateGauge = Metrics.CreateGauge("HashStormCore_pool_hashrate", "Hashrate per pool", new GaugeConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        btStreamLatencySummary = Metrics.CreateSummary("HashStormCore_btstream_latency", "Latency of streaming block-templates in ms", new SummaryConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        shareCounter = Metrics.CreateCounter("HashStormCore_shares_total", "Received shares per pool", new CounterConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        validShareCounter = Metrics.CreateCounter("HashStormCore_valid_shares_total", "Valid received shares per pool", new CounterConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        invalidShareCounter = Metrics.CreateCounter("HashStormCore_invalid_shares_total", "Invalid received shares per pool", new CounterConfiguration
        {
            LabelNames = new[] { "pool" }
        });

        rpcRequestDurationSummary = Metrics.CreateSummary("HashStormCore_rpcrequest_execution_time", "RPC request execution time ms", new SummaryConfiguration
        {
            LabelNames = new[] { "pool", "method" }
        });

        stratumRequestDurationSummary = Metrics.CreateSummary("HashStormCore_stratum_request_execution_time", "Stratum request execution time ms", new SummaryConfiguration
        {
            LabelNames = new[] { "pool", "method" }
        });

        apiRequestDurationSummary = Metrics.CreateSummary("HashStormCore_api_request_execution_time", "API request execution time ms", new SummaryConfiguration
        {
            LabelNames = new[] { "request" }
        });

        hashComputationSummary = Metrics.CreateSummary("HashStormCore_hash_computation_time", "Hash computation time ms", new SummaryConfiguration
        {
            LabelNames = new[] { "algo" }
        });
    }
