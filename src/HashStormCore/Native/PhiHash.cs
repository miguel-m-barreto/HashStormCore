...
public static unsafe class PhiHash
{
    [DllImport("libphihash", EntryPoint = "ethash_create_epoch_context", CallingConvention = CallingConvention.Cdecl)]
    public static extern IntPtr CreateContext(int epoch_number);

    [DllImport("libphihash", EntryPoint = "ethash_destroy_epoch_context", CallingConvention = CallingConvention.Cdecl)]
    public static extern void DestroyContext(IntPtr context);

    [DllImport("libphihash", EntryPoint = "hash", CallingConvention = CallingConvention.Cdecl)]
    public static extern Ethash_result hash(IntPtr context, int block_number, ref Ethash_hash256 header_hash, ulong nonce);

    [DllImport("libphihash", EntryPoint = "ethash_calculate_epoch_seed", CallingConvention = CallingConvention.Cdecl)]
