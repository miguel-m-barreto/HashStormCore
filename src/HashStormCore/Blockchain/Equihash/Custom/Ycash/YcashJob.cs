// src/HashStormCore/Blockchain/Equihash/Custom/Ycash_Job.cs
using System;
...
using NBitcoin;
/*
namespace HashStormCore.Blockchain.Equihash.Custom.Ycash
{
    /// <summary>
    ...
    public class YcashJob : EquihashJob
    {
            ...
            IDestination poolAddressDestination, Network network, EquihashSolver solver)
        {
            ...
            // Priority: cluster coinbase string -> pool template comment -> default "HashStormCore".
            var tag = clusterConfig?.PaymentProcessing?.CoinbaseString;
            if (string.IsNullOrWhiteSpace(tag))
                tag = poolConfig?.Template?.CoinbaseTxComment ?? "HashStormCore";
