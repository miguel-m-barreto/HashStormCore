...
using Microsoft.IO;
using HashStormCore.Blockchain.Zano.StratumRequests;
using HashStormCore.Blockchain.Zano.StratumResponses;
...
namespace HashStormCore.Blockchain.Zano;
...
public class ZanoPool : PoolBase
{
    ...
    private async Task OnSubmitAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            logger.Debug(() => $"[{connection.ConnectionId}] Share accepted: D={Math.Round(share.Difficulty * coin.ShareMultiplier, 3)}");
