...
using Microsoft.IO;
using HashStormCore.Blockchain.Cryptonote.StratumRequests;
using HashStormCore.Blockchain.Cryptonote.StratumResponses;
...
namespace HashStormCore.Blockchain.Cryptonote;
...
public class CryptonotePool : PoolBase
{
    ...
    private async Task OnSubmitAsync(StratumConnection connection, Timestamped<JsonRpcRequest> tsRequest, CancellationToken ct)
    {
        ...
        try
        {
            ...
            logger.Debug(() => $"[{connection.ConnectionId}] Share accepted: D={Math.Round(share.Difficulty, 3)}");
    ...
    private static void ValidateSubmitFields(CryptonoteSubmitShareRequest submitRequest)
    {
        if(submitRequest == null)
            throw new StratumException(StratumError.MinusOne, "invalid request");

        if(string.IsNullOrEmpty(submitRequest.Nonce) || !CryptonoteConstants.RegexValidNonce.IsMatch(submitRequest.Nonce))
